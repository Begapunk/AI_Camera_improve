...
export class TemplatePoseError extends Error {
  constructor(message, { localPath = null, recoverable = false } = {}) {
    super(message);
...
export function extractLandmarksFromLocalPath(localPath) {
  return detectPoseApi(localPath).then((uploadRes) => {
    if (uploadRes.statusCode !== 200) {
      ...
      throw new TemplatePoseError('姿态识别服务异常，请重试', { localPath, recoverable: false });
    }
    const kpArray = uploadRes.data?.keypoints;
    if (!kpArray) {
      // 需求2：真没检测到人体——常见于二次元/动漫人物、纯风景图等 YOLOv8-pose 认不出的画风
      throw new TemplatePoseError('模板图中未检测到人体骨骼', { localPath, recoverable: true });
    }
    ...
    if (missing.length > 0) {
      ...
      throw new TemplatePoseError('模板图人物姿态关键部位未检测完整', { localPath, recoverable: true });
    }
...
export function fetchTemplateLandmarks(imageUrl) {
  if (!imageUrl) {
    return Promise.reject(new TemplatePoseError('模板图片地址不能为空'));
  }
  ...
  return new Promise((resolve, reject) => {
    uni.downloadFile({
      ...
      fail: () => {
        reject(new TemplatePoseError('模板图片下载失败，请检查网络'));
      },
    });
